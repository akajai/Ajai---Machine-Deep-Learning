...
- [ ] A) To create a model that is as complex as possible.
- [ ] B) To predict an outcome by learning simple decision rules from data.
- [ ] C) To only handle numerical data.
- [ ] D) To create a "black box" model.

**Correct Answer: B**

**Explanation:**
- **A) To create a model that is as complex as possible.** This is incorrect. Overly complex trees are prone to overfitting. The goal is to create a simple, generalizable model.
...
- **D) To create a "black box" model.** This is incorrect. Decision trees are known as "white box" models because their decision-making process is transparent and easy to interpret.
...
- [ ] D) The process of dividing a node.

**Correct Answer: C**

**Explanation:**
- **A) The starting point of the tree.** This describes the **root node**.
...
- **D) The process of dividing a node.** This describes **splitting**.
...
- [ ] D) They can only be used for classification.

**Correct Answer: C**

**Explanation:**
- **A) They are difficult to interpret.** This is false; their main strength is ease of interpretation.
...
- **D) They can only be used for classification.** This is false; they can be used for both classification and regression tasks.
...
- [ ] D) The number of features used in the tree.

**Correct Answer: C**

**Explanation:**
- **A) The total number of nodes in the tree.** This is the size of the tree, not the depth of a specific node.
...
- **D) The number of features used in the tree.** This is unrelated to the concept of depth.
...
**Explanation:**
- **A) Increase the model's complexity.** Setting a `max_depth` *limits* complexity, it doesn't increase it.
...
- **D) Handle imbalanced data.** While related to performance, `max_depth` doesn't directly address data imbalance. Other techniques like class weighting are used for that.
...
**Explanation:**
- **A) Classification Tree.** This type of tree predicts a discrete category (e.g., "Spam" or "Not Spam").
...
- **D) Greedy Tree.** This describes the *process* of building a tree (greedy search), not its type based on the output.

---

**7. The process of building a Decision Tree is described as a "greedy recursive process." What does the "greedy" part mean?**
- [ ] A) The algorithm plans several steps ahead to find the globally optimal tree.
...
- [ ] D) The algorithm continues splitting until every node is 100% pure.

**Correct Answer: B**

**Explanation:**
- **A) The algorithm plans several steps ahead...** This is the opposite of a greedy approach.
...
- **D) The algorithm continues splitting until every node is 100% pure.** While a greedy process *can* lead to this, the "greedy" part specifically refers to the step-by-step decision making, not the stopping criterion.
...
**Explanation:**
- **A) Information Gain.** This measures the *reduction* in impurity after a split, not the impurity itself.
...
- **D) Pruning.** This is a technique to reduce overfitting, not a measure of impurity.

---

**9. A node has a Gini Impurity of 0. What does this signify?**
- [ ] A) The node is perfectly mixed (50/50 split).
- [ ] B) The node is the root node.
- [ ] C) The node is perfectly pure (all items belong to one class).
- [ ] D) The node requires further splitting.

**Correct Answer: C**

**Explanation:**
- **A) The node is perfectly mixed (50/50 split).** This would result in the maximum Gini Impurity (0.5 for a two-class problem).
...
- **D) The node requires further splitting.** A pure node is a leaf node and does not need to be split further.

---

**10. How does a Regression Tree make a prediction for a new data point?**
- [ ] A) It predicts the majority class of the training data in the leaf node.
...
- [ ] D) It predicts the median of all target values in the dataset.

**Correct Answer: B**

**Explanation:**
- **A) It predicts the majority class...** This is how a **classification tree** makes a prediction.
...
- **D) It predicts the median of all target values in the dataset.** This would be a very naive model that ignores all features.
...
**Explanation:**
- **A) Information Gain.** Used for classification trees.
...
- **D) Chi-Square.** Another metric sometimes used for classification, but Variance Reduction is standard for regression.
...
**Explanation:**
- **A) 0.** This is the minimum value, representing a perfectly pure node.
...
- **D) It has no upper limit.** This is incorrect; it is bounded.

---

**13. Which statement is true when comparing Gini Impurity and Entropy?**
- [ ] A) Entropy is computationally faster to calculate than Gini Impurity.
...
- [ ] D) Using Entropy always results in a more accurate tree than using Gini Impurity.

**Correct Answer: C**

**Explanation:**
- **A) Entropy is computationally faster...** This is false. The logarithm calculation in Entropy makes it slower than the squaring operation in Gini Impurity.
...
- **D) Using Entropy always results in a more accurate tree...** This is false. In practice, the difference in performance is usually negligible.
...
**Explanation:**
- **A) Splitting.** This is the process of creating branches, not removing them.
...
- **D) Rooting.** This refers to establishing the first node of the tree.
...
- [ ] B) High purity
- [ ] C) Zero impurity
- [ ] D) A Gini Impurity of 0.5

**Correct Answer: A**

**Explanation:**
- **A) Low purity.** This is correct. The node is quite mixed, with a 60/40 split. This means it has high impurity and low purity.
...
- **D) A Gini Impurity of 0.5.** This would only be true if the split was exactly 50/50 (500 approved, 500 denied). The Gini score here would be `1 - (0.6^2 + 0.4^2) = 1 - (0.36 + 0.16) = 0.48`, which is high, but not the maximum. "Low purity" is the best description.

---

**16. Which of these is an advantage of Decision Trees?**
- [ ] A) They are very stable; small changes in data do not affect the tree.
- [ ] B) They are immune to the effects of imbalanced data.
- [ ] C) They require little data preparation (no need for normalization).
- [ ] D) They are guaranteed to find the globally optimal model.

**Correct Answer: C**

**Explanation:**
- **A) They are very stable...** This is false. Decision trees are known to be unstable; small data changes can lead to a completely different tree.
...
- **D) They are guaranteed to find the globally optimal model.** This is false. The greedy approach means they find a locally optimal solution, which may not be the globally best one.
...
**Explanation:**
- **A) Underfitting.** This occurs when a model is too simple (e.g., a very shallow tree).
...
- **D) High bias and low variance.** This describes an underfit model. An overfit model has low bias and high variance.

---

**19. In a classification tree, the algorithm splits a node based on the feature that:**
- [ ] A) Results in the highest increase in impurity.
- [ ] B) Results in the greatest reduction in impurity.
- [ ] C) Has the most unique values.
- [ ] D) Is a numerical feature.

**Correct Answer: B**

**Explanation:**
- **A) Results in the highest increase in impurity.** The goal is to decrease impurity, not increase it.
...
- **D) Is a numerical feature.** The best split can come from either numerical or categorical features.

---

**20. A "white box" model means that:**
- [ ] A) The model is a neural network.
...
- [ ] D) The model can only be trained on text data.

**Correct Answer: C**

**Explanation:**
- **A) The model is a neural network.** Neural networks are typically considered "black box" models.
...
- **D) The model can only be trained on text data.** This is unrelated to the "white box" concept.

---

**21. A decision tree is considered "non-parametric" because:**
- [ ] A) It has no parameters to tune.
...
**Explanation:**
- **A) It has no parameters to tune.** This is false. It has hyperparameters like `max_depth` and `min_samples_leaf`.
...
- **D) It is not a machine learning model.** This is false.

---

**22. What is the role of the `min_samples_split` hyperparameter?**
- [ ] A) It sets the minimum number of samples required to be at a leaf node.
- [ ] B) It sets the maximum depth of the tree.
- [ ] C) It sets the minimum number of samples required to split an internal node.
- [ ] D) It determines the impurity measure to use.

**Correct Answer: C**

**Explanation:**
- **A) It sets the minimum number of samples required to be at a leaf node.** This describes the `min_samples_leaf` hyperparameter.
...
- **D) It determines the impurity measure to use.** This is determined by the `criterion` hyperparameter (e.g., 'gini' or 'entropy').

---

**23. If a split on a feature results in an Information Gain of 0, what does this imply?**
- [ ] A) The split is the best possible split.
- [ ] B) The split resulted in perfectly pure child nodes.
- [ ] C) The split provided no new information about the classification.
- [ ] D) The feature is a categorical feature.

**Correct Answer: C**

**Explanation:**
- **A) The split is the best possible split.** This is incorrect. An Information Gain of 0 means it's a useless split.
...
- **D) The feature is a categorical feature.** The type of feature is irrelevant to the information gain value.
...
- [ ] D) The number of samples in the node

**Correct Answer: C**

**Explanation:**
- **A) Gini Impurity.** Used for classification.
...
- **D) The number of samples in the node.** This is a property of the node, not a measure of its impurity.
...
**Explanation:**
- **A) Stability.** This is the opposite of the described behavior.
...
- **D) Bias.** Bias refers to the simplifying assumptions made by a model, not its reaction to data changes.

---

**26. How do decision trees handle missing values in data?**
- [ ] A) They automatically impute the mean/median for the missing values.
- [ ] B) They cannot handle missing values and will raise an error.
- [ ] C) Some algorithms can ignore them or use surrogate splits.
- [ ] D) They treat missing values as a separate category.

**Correct Answer: C**

**Explanation:**
- **A) They automatically impute the mean/median...** This is a separate preprocessing step, not an inherent capability of all tree algorithms.
...
- **C) Some algorithms can ignore them or use surrogate splits.** This is correct. Advanced tree algorithms like CART can use "surrogate splits" (alternative splits) when a value is missing. Others might distribute the instance down multiple branches.
- **D) They treat missing values as a separate category.** While this is a possible preprocessing strategy, it's not the only way trees handle them.

---

**27. What is the "bias-variance trade-off" in the context of a decision tree's `max_depth`?**
- [ ] A) A shallow tree has high bias and high variance.
- [ ] B) A deep tree has low bias and low variance.
- [ ] C) A shallow tree has high bias and low variance.
- [ ] D) A deep tree has high bias and high variance.

**Correct Answer: C**

**Explanation:**
- **A) A shallow tree has high bias and high variance.** Incorrect. Low variance.
...
- **D) A deep tree has high bias and high variance.** Incorrect. Low bias.
...
**Explanation:**
- **A) Leaf Node.** This is a terminal node with a final decision.
...
- **D) Branch.** This is the connection between nodes.

---

**29. Which of the following is a method of "pre-pruning" a decision tree?**
- [ ] A) Growing the tree to its full depth and then removing branches.
- [ ] B) Setting a minimum value for information gain to make a split.
- [ ] C) Using a separate validation set to evaluate the tree.
- [ ] D) Converting the tree into a set of rules.

**Correct Answer: B**

**Explanation:**
- **A) Growing the tree to its full depth and then removing branches.** This describes **post-pruning**.
...
- **D) Converting the tree into a set of rules.** This is a way to interpret the tree, not to prune it.
...
**Explanation:**
- **A) The correlation between two variables.** This is typically shown with a scatter plot.
...
- **D) A neural network architecture.** This is represented by a graph of neurons and layers.
...
**Explanation:**
- **A) They are easier to interpret.** This is false. Ensembles of trees are much harder to interpret than a single tree.
...
- **D) They require less data.** They typically require more data to be effective.

---

**33. A split on a numerical feature (e.g., Age) in a decision tree involves:**
- [ ] A) Creating a branch for every unique value of Age.
...
- [ ] D) Ignoring the feature because it is not categorical.

**Correct Answer: B**

**Explanation:**
- **A) Creating a branch for every unique value...** This is inefficient and leads to overfitting. It's typically done for categorical features.
...
- **D) Ignoring the feature...** Decision trees are well-suited for numerical features.

---

**34. The term "recursive partitioning" refers to:**
- [ ] A) The final prediction made by the tree.
- [ ] B) The process of repeatedly splitting the dataset into smaller and smaller subsets.
- [ ] C) The calculation of Gini Impurity.
- [ ] D) The pruning of the decision tree.

**Correct Answer: B**

**Explanation:**
- **A) The final prediction made by the tree.** This is the output, not the process.
...
- **D) The pruning of the decision tree.** This is a separate step to control complexity.
...
**Explanation:**
- **A) Underfitting.** This would result in low accuracy on *both* training and test data.
...
- **D) Data leakage.** While this can cause high performance, overfitting is the more direct description of the performance gap between training and testing.
...
- [ ] D) Both Gini and Entropy are equally sensitive.

**Correct Answer: B**

**Explanation:**
- **A) Gini Impurity.** Less sensitive due to its parabolic shape.
...
- **D) Both Gini and Entropy are equally sensitive.** This is incorrect; there is a slight difference in their sensitivity curves.
...
**Explanation:**
- **A) A smooth curve.** This is characteristic of models like Support Vector Machines with non-linear kernels.
...
- **D) A circle.** This is not a typical decision boundary for a tree.

---

**38. What happens if you set `min_samples_leaf` to a very high value?**
- [ ] A) The tree will be more likely to overfit.
- [ ] B) The tree will be more likely to underfit.
- [ ] C) The tree will become more complex.
- [ ] D) It will have no effect on the tree's structure.

**Correct Answer: B**

**Explanation:**
- **A) The tree will be more likely to overfit.** This is incorrect. Overfitting is associated with low values for `min_samples_leaf`.
...
- **D) It will have no effect...** This is incorrect; it's a key hyperparameter.

---

**39. The CART (Classification and Regression Trees) algorithm uses which of the following for splitting?**
- [ ] A) Information Gain for classification and Variance Reduction for regression.
...
- [ ] D) Information Gain for both classification and regression.
...
- [ ] A) Are less prone to overfitting.
...
**Explanation:**
- **A) Are less prone to overfitting.** This is false; they are more prone to overfitting.
...
- **D) Require feature scaling.** This is false; they do not require it.

---

**41. What is a "surrogate split"?**
- [ ] A) A split that results in zero information gain.
...
- [ ] D) A split based on a randomly chosen feature.

**Correct Answer: B**

**Explanation:**
- **A) A split that results in zero information gain.** This is just a poor split.
...
- **D) A split based on a randomly chosen feature.** This is a technique used in Random Forests, not a surrogate split.

---

**42. If a feature is very important for a decision tree model, it will likely be used:**
- [ ] A) Only in the deepest parts of the tree.
- [ ] B) Near the root of the tree.
- [ ] C) In multiple branches simultaneously.
- [ ] D) To create the final leaf nodes.

**Correct Answer: B**

**Explanation:**
- **A) Only in the deepest parts of the tree.** Less important features are used deeper in the tree.
...
- **D) To create the final leaf nodes.** Splits create decision nodes, not leaf nodes.
...
- **B) Overlapping and complex.** The rules are not overlapping.
- **C) Always linear.** The rules are logical, not necessarily linear.
- **D) Difficult to understand.** The rules are typically very easy to understand.

---

**44. In a two-class problem, a node has an Entropy of 1. This means the node contains:**
- [ ] A) Only samples from Class A.
- [ ] B) Only samples from Class B.
- [ ] C) An equal number of samples from Class A and Class B.
- [ ] D) More samples from Class A than Class B.

**Correct Answer: C**

**Explanation:**
- **A) Only samples from Class A.** This would be an Entropy of 0.
...
- **D) More samples from Class A than Class B.** This would result in an Entropy value between 0 and 1, but not 1.
...
- [ ] D) It produces trees that are too simple.

**Correct Answer: B**

**Explanation:**
- **A) It is computationally very expensive.** The greedy approach is actually a heuristic to make the problem computationally feasible. Finding the truly optimal tree is an NP-hard problem.
...
- [ ] D) It produces trees that are too simple. It often produces trees that are too complex and overfit.
...
**Explanation:**
- **A) CART.** CART was developed concurrently with ID3 and has its own lineage.
...
- **D) MARS.** This is a different type of regression algorithm.
...
- [ ] A) The tree cannot handle categorical features.
- [ ] B) The feature will cause the tree to underfit.
- [ ] C) The tree might create a large number of branches, leading to overfitting.
- [ ] D) The feature has no predictive power.

**Correct Answer: C**

**Explanation:**
- **A) The tree cannot handle categorical features.** This is false.
...
- **D) The feature has no predictive power.** We cannot know this without testing it, but the high cardinality itself is a structural problem.

---

**48. Post-pruning a decision tree involves:**
- [ ] A) Setting `max_depth` before training.
- [ ] B) Growing the tree fully, then removing or collapsing nodes.
- [ ] C) Using Gini Impurity instead of Entropy.
- [ ] D) Stopping the tree growth early.

**Correct Answer: B**

**Explanation:**
- **A) Setting `max_depth` before training.** This is pre-pruning.
...
- **D) Stopping the tree growth early.** This is pre-pruning.
...
**Explanation:**
- **A, B, C.** These are all distinct machine learning algorithms.
- **D) Random Forest.** This is correct. A Random Forest is an ensemble model that builds and aggregates the results from a multitude of individual decision trees to produce a more robust and accurate prediction.
