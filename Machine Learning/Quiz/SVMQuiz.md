...
- [ ] A) To find a hyperplane that passes through the maximum number of data points.
...
- [ ] D) To find a hyperplane that is perpendicular to the support vectors.
...
- **A)** The hyperplane should separate the classes, not pass through the data points.
- **B)** SVM aims to maximize, not minimize, the margin to create a more robust classifier.
- **D)** The hyperplane is defined by the support vectors, but it's not necessarily perpendicular to them in a simple sense.
...
- [ ] D) The data points that are used to calculate the mean of each class.
...
- **A)** Only a subset of data points are support vectors.
- **B)** Points farthest from the hyperplane have no influence on it.
- **D)** This describes a different type of classifier, like one based on centroids.

**3. What is the "margin" in the context of SVM?**
- [ ] A) The distance from the hyperplane to the origin.
...
- [ ] D) The total width of the "road" separating the two classes, defined by the support vectors.
...
- [ ] D) When using a non-linear kernel.
...
- **A, C)** For non-linearly separable or noisy data, a Soft Margin SVM is required.
- **D)** Non-linear kernels are used for non-linearly separable data, which is a scenario for Soft Margin SVMs.
...
- [ ] D) To calculate the distance between two data points.
...
- [ ] A) It controls the width of the Gaussian kernel.
...
- [ ] D) It is the learning rate of the optimization algorithm.
...
- [ ] A) A method for speeding up the training of a linear SVM.
...
- [ ] D) A method for choosing the best value for the `C` parameter.
...
- [ ] A) By calculating the distance to the nearest support vector.
...
- [ ] D) By using a majority vote of the support vectors.
...
- **A)** The computational cost can be high, especially with non-linear kernels.
- **C)** They can be used in spaces of any dimension.
- **D)** SVMs do not inherently perform feature selection.

**14. What happens if you move a data point that is *not* a support vector?**
- [ ] A) The hyperplane will shift.
- [ ] B) The margin will become smaller.
- [ ] C) The margin will become larger.
- [ ] D) The hyperplane will not change.
...
- **B, C, D)** These are incorrect interpretations.

**17. Which of the following is a potential disadvantage of SVMs?**
- [ ] A) They are not effective for non-linear data.
...
- **A)** The kernel trick makes them very effective for non-linear data.
- **B)** Soft Margin SVMs are designed to handle noisy data.
- **D)** They are well-suited for high-dimensional data.

**18. What is the role of the bias term `b` in the hyperplane equation?**
- [ ] A) It controls the orientation of the hyperplane.
...
- **A)** The orientation is controlled by the weight vector `w`.
- **C)** The margin width is determined by ||w||.
- **D)** It is not always zero.
...
- [ ] D) No decision boundary can be formed.
...
- [ ] A) SVM is a supervised algorithm, while Logistic Regression is unsupervised.
...
- **D)** Both are linear models, but can be extended to non-linear problems using kernels (for SVM) or feature engineering (for Logistic Regression).

**21. What does it mean if a data point has a slack variable (ξ) value of 0?**
- [ ] A) The data point is misclassified.
...
- [ ] D) The data point is a support vector.
...
- **A, B)** These would result in a positive slack variable.
- **D)** While it could be a support vector on the margin, it's not necessarily one.

**22. The "support" in Support Vector Machine refers to:**
- [ ] A) The support from the open-source community.
...
- [ ] D) The mathematical proof that supports the algorithm's convergence.
...
- [ ] A) When the data is perfectly linearly separable.
...
- **A)** A Hard Margin SVM would be suitable here.
- **B, D)** The choice between hard and soft margin is primarily about data separability, not dataset size or efficiency.
...
- [ ] A) They are very fast to train on text data.
...
- [ ] D) They automatically handle stop words and stemming.
...
- **A)** Training can be slow on large text datasets.
- **C, D)** SVMs do not have built-in natural language understanding capabilities; these tasks require separate preprocessing steps.

**26. If you have a dataset with 3 classes, how would you typically use SVM for classification?**
- [ ] A) SVMs cannot be used for more than 2 classes.
- [ ] B) Use a one-vs-one or one-vs-all approach.
- [ ] C) Use a single hyperplane to separate all 3 classes at once.
- [ ] D) Use a kernel function that is specifically designed for 3 classes.
...
- **A, B)** These are not always true.
- **D)** SVM does not directly provide a probability distribution.

**28. What is the primary reason for maximizing the margin in SVM?**
- [ ] A) To improve the model's generalization to new data.
...
- **B, C, D)** These are not the primary reasons for maximizing the margin.
