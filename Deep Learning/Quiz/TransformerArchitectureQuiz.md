...
- [ ] B) The self-attention mechanism allows direct connections between any two tokens in the sequence, removing the need for sequential processing.
- [ ] C) They break the sentence into individual characters instead of words.
- [ ] D) They require significantly less memory, allowing for parallel computation.

**Correct Answer:** B

**Explanation:**
- A) Incorrect. The activation functions used (like ReLU and Softmax) are standard and not the reason for parallelization.
- B) Correct. RNNs must process token by token to maintain sequential context. The self-attention mechanism in Transformers calculates the relationships between all tokens simultaneously, eliminating this sequential dependency and enabling parallel processing.
- C) Incorrect. Tokenization strategy is separate from the core architectural difference that allows parallelization.
- D) Incorrect. Transformers, especially large ones, are very memory-intensive; this is not an advantage they have over RNNs.

**2. In the self-attention mechanism, what are the three vectors created for each input token?**
- [ ] A) Input, Hidden, and Output
- [ ] B) Query, Key, and Value
- [ ] C) Position, Embedding, and Context
- ] D) Word, Subword, and Character

**Correct Answer:** B

**Explanation:**
- A) Incorrect. These are general neural network concepts, not the specific vectors used in the self-attention calculation.
...
- D) Incorrect. These are levels of tokenization, not the vectors used in self-attention.

**3. How is the initial similarity between two tokens calculated in the scaled dot-product attention?**
- [ ] A) By taking the dot product of their Value vectors.
- [ ] B) By passing their embeddings through a separate neural network.
- [ ] C) By taking the dot product of the current token's Query vector and the other token's Key vector.
- [ ] D) By measuring the Euclidean distance between their positional encodings.

**Correct Answer:** C

**Explanation:**
- A) Incorrect. The Value vectors are used at the end of the calculation to create the output, not at the beginning to calculate similarity.
...
- C) Correct. The dot product between a token's Query (its question) and another token's Key (its label) produces a raw similarity score, indicating their relevance to each other.
- D) Incorrect. Positional encodings are added to the embeddings but are not directly used for the similarity calculation in this manner.

**4. What is the main purpose of the "mask" in the Masked Multi-Head Self-Attention layer of the Transformer decoder?**
- [ ] A) To prevent the model from paying attention to padding tokens.
- [ ] B) To randomly deactivate neurons to prevent overfitting.
- [ ] C) To prevent the decoder from "cheating" by attending to future tokens in the output sequence during training.
- [ ] D) To hide the Keys and Values from the Query vectors.

**Correct Answer:** C

**Explanation:**
- A) Incorrect. While a padding mask is also used, the specific purpose of the mask in this decoder layer is to handle future tokens, not padding.
- B) Incorrect. This describes Dropout, which is a different regularization technique.
- C) Correct. Since the decoder generates the output word by word (autoregressively), it should only be allowed to use the words it has already generated as context. The mask sets the attention scores for all future positions to negative infinity, so they become zero after the Softmax.
- D) Incorrect. The mask is applied to the attention scores, not the Keys or Values directly.
...
- [ ] B) Because the self-attention mechanism itself has no inherent sense of word order or position.
- [ ] C) To normalize the input values before they enter the encoder.
- [ ] D) To convert words into their subword tokens.

**Correct Answer:** B

**Explanation:**
- A) Incorrect. While it is added to the embeddings, its purpose is to encode position, not just increase dimensionality.
- B) Correct. The self-attention mechanism treats the input as a "bag of words," calculating the relationship between every pair of tokens regardless of their position. Positional Encoding explicitly injects information about the position of each token into the model.
- C) Incorrect. This is the role of Layer Normalization.
- D) Incorrect. This is the role of the tokenizer.

**6. What is the primary function of the Cross-Attention layer in the Transformer decoder?**
- [ ] A) To allow the decoder to attend to the tokens it has previously generated.
...
- [ ] D) To combine the outputs of the multiple attention heads.

**Correct Answer:** B

**Explanation:**
- A) Incorrect. This is the function of the Masked Self-Attention layer in the decoder.
...
- C) Incorrect. It often results in more tokens per sentence, not fewer.
...
- D) Incorrect. This analogy does not fit the Q, K, V framework.

**9. What is the purpose of the Feed-Forward Network (MLP) sub-layer within each Transformer block?**
- [ ] A) To calculate the attention scores between tokens.
...
- [ ] D) To combine the multiple attention heads into a single vector.

**Correct Answer:** B

**Explanation:**
- A) Incorrect. This is done by the self-attention layer.
...
- C) Incorrect. This is the role of Layer Normalization.
- D) Incorrect. This is done by a linear projection after the multi-head attention calculation.

**10. How does Multi-Head Self-Attention (MHSA) enhance the Transformer's capabilities?**
- [ ] A) It allows the model to process multiple sentences at the same time.
...
**Explanation:**
- A) Incorrect. Batching allows for processing multiple sentences, not MHSA itself.
...
- D) Incorrect. Positional encodings are still required.

**11. What is the out-of-vocabulary (OOV) problem?**
- [ ] A) When a model produces a word that is not in the dictionary.
...
- [ ] D) When a word has multiple meanings (polysemy).

**Correct Answer:** B

**Explanation:**
- A) Incorrect. This is a generation issue, not the OOV problem itself.
...
- D) Incorrect. This is a problem of ambiguity, which attention helps to solve.
...
- [ ] A) It is used to separate two different sentences in the input.
...
- ] D) It is a padding token used to make all sequences in a batch the same length.

**Correct Answer:** B

**Explanation:**
- A) Incorrect. This is the role of the `[SEP]` token.
...
- D) Incorrect. This is the role of the `[PAD]` token.

**14. In the Transformer decoder, where do the Keys and Values for the Cross-Attention layer come from?**
- [ ] A) The output of the previous masked self-attention layer in the decoder.
- [ ] B) The output of the final encoder block.
- [ ] C) The target output sequence (the ground truth).
- [ ] D) A separate, randomly initialized set of vectors.

**Correct Answer:** B

**Explanation:**
- A) Incorrect. The Queries come from the decoder's previous layer, but not the Keys and Values.
...
- D) Incorrect. The Keys and Values must contain the contextual information from the input sentence.

**15. What is the purpose of the final Softmax layer in the Transformer decoder?**
- [ ] A) To calculate the attention weights for the cross-attention layer.
...
**Explanation:**
- A) Incorrect. Softmax is used within the attention layers, but the final Softmax has a different purpose.
...
- D) Incorrect. This is done within the Multi-Head Attention block.

**16. What problem does the scaling factor ($$\frac{1}{\sqrt{d_k}}$$) in the scaled dot-product attention formula solve?**
- [ ] A) It prevents the dot products from growing too large in magnitude, which would push the Softmax function into regions with extremely small gradients.
- [ ] B) It normalizes the Value vectors before they are summed.
- [ ] C) It ensures that the attention weights sum to 1.
- [ ] D) It reduces the computational cost of the matrix multiplication.

**Correct Answer:** A

**Explanation:**
- A) Correct. For large values of the key dimension ($$d_k$$), the dot products can become very large. This can saturate the Softmax function, leading to vanishing gradients and hindering the learning process. The scaling factor counteracts this effect.
...
- D) Incorrect. It is an element-wise scaling and does not significantly change the computational cost.

**17. How does the WordPiece algorithm decide which subwords to merge when building its vocabulary?**
- [ ] A) It merges the longest possible subwords first.
...
- [ ] D) It merges tokens randomly to increase diversity.

**Correct Answer:** B

**Explanation:**
- A) Incorrect. The greedy approach is used when tokenizing a word, not when building the vocabulary.
...
- D) Incorrect. The process is deterministic and based on frequency counts.

**18. What is the role of a padding mask in Transformer training?**
- [ ] A) To prevent the model from attending to future positions in the decoder.
...
- ] D) To mask out the Query vectors in the self-attention calculation.

**Correct Answer:** B

**Explanation:**
- A) Incorrect. This is the role of the look-ahead (or causal) mask in the decoder.
...
- D) Incorrect. The mask is applied to the attention scores before the Softmax, not to the Query vectors.
...
- [ ] D) The use of subword tokenization.

**Correct Answer:** C

**Explanation:**
- A) Incorrect. The MLP is for non-linear processing, not for solving the information bottleneck.
...
- D) Incorrect. Tokenization solves the OOV problem, not the information bottleneck.

**20. In a machine translation task, the Cross-Attention layer helps the decoder to...**
- [ ] A) ...understand the grammar of the output language.
- [ ] B) ...learn the alignment between words in the source and target languages.
- [ ] C) ...process the source sentence in parallel.
- [ ] D) ...generate a vocabulary for the target language.

**Correct Answer:** B

**Explanation:**
- A) Incorrect. This is learned more broadly by the entire decoder, particularly the masked self-attention.
...
- D) Incorrect. The vocabulary is predefined before training.
